...
public class PDFMaker {
    ...
    public static void generateInvoicePDF(List<SaleEntity> sales) {
        ...
        if (!sales.isEmpty()) {
            ...
            sales.forEach(sale ->{
                ...
                try {
                    String fileName = "src\\test\\resources\\cupons\\invoice_" + sale.getId() + ".pdf";
                    ...
                    document.add(new Paragraph("CPF/CNPJ: " + Utils.formatCPF(sale.getCustomer().getCpf())));
                    ...
                    // loop items
                    int i = 1;
                    for (SaleProductEntity item : sale.getSaleproducts()) {
                        ...
                        PdfPCell cell8 = new PdfPCell(new Phrase(item.getProduct().getId() + "", tblheadfont));
                        tbhi.addCell(cell8);

                        PdfPCell cell9 = new PdfPCell(new Phrase(item.getProduct().getName(), tblheadfont));
                        tbhi.addCell(cell9);
